...
class BinanceUserDataClient:
    ...
    def handle_message(self, message: dict) -> None:
        ...
        if event_type == "ORDER_TRADE_UPDATE":
            ...
            logging.info(
                "User data update received: %s",
                event_type,
            )

    def handle_order_trade_update(self, message: dict) -> None:
        trade_data = message["o"]
        symbol = trade_data["s"]
        side = trade_data["S"]
        order_type = trade_data["o"]
        execution_type = trade_data["x"]
        order_status = trade_data["X"]
        last_filled_price = trade_data["L"]
        last_filled_quantity = trade_data["l"]
        cumulative_filled_quantity = trade_data["z"]
        average_price = trade_data["ap"]
        logging.info(
            "Order update | symbol=%s side=%s type=%s "
            "executionType=%s orderStatus=%s "
            "lastFillPrice=%s lastFillQty=%s "
            "cumQty=%s avgPrice=%s",
            symbol,
            side,
            order_type,

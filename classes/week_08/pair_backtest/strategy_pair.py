...
class PairTradingStrategy:
    ...
    def evaluate_pair(self) -> None:
        ...
        if not self._is_ready_to_sample():
            return
    ...
    def _trade_symbol_to_target(self, symbol: str, target_position: float) -> bool:
        ...
        self.has_pending_order[symbol] = True
        ...
        if not order_sent:
            self.has_pending_order[symbol] = False
    ...
    def _is_ready_to_sample(self) -> bool:
        base_timestamp = self.last_book[self.base_symbol].timestamp
        hedge_timestamp = self.last_book[self.hedge_symbol].timestamp

        if self.sample_interval_seconds == 0:
            if base_timestamp != hedge_timestamp:
                return False

            if self.last_sample_timestamp == base_timestamp:
                return False

            self.last_sample_timestamp = base_timestamp
            return True
